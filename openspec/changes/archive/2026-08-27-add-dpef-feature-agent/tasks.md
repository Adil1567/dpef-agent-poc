...
- [x] 6.1 Write `.github/workflows/verify-dpef-state.yml`: triggered automatically by GitHub on every push to a PR branch — no manual or agent-initiated trigger. It re-fetches the PR's real approval/merge status from the GitHub API and compares it against the `dpef-agent/state/<feature>.yaml` committed in that branch; fails (red X on the PR) if they disagree
- [x] 6.2 You (repo admin) mark this check as required in the target repo's branch protection settings, and separately enable "require approving review before merge" — this makes merging physically blocked while the check is red, so nothing needs to separately "notify" anyone of a mismatch after the fact
- [x] 6.3 Document that this CI check only guards the merge step, not the deploy step — the deploy step's own guard is the synchronous check in task 5.4, done by the agent at the moment it acts

## 7. Evaluation

- [x] 7.1 Write test cases covering: a comment alone never changes state (only explicit confirm-and-push records a correction); a deploy instruction before merge is refused; a deploy instruction with a deliberately stale "merged" state file gets corrected and still refused; multiple features stay independent; illegal transitions rejected. Implemented as pytest (`dpef-agent/scripts/test_state_helper.py`, `uv add --dev pytest`) rather than `claude plugin eval`, since the skill isn't packaged as a plugin. Covers the deterministic state-logic core; comment-classification/preview-confirmation judgment calls are covered by the manual checks below. 8/8 pass.
- [x] 7.2 Manually verify: a feature request produces a preview (not a PR) and the PR opens only after explicit confirmation — confirmed live end-to-end via `dpef:build-feature` with a real "add a README.md" request: state file created at `developing`, preview shown, no PR opened until explicit "yes", then PR #2 opened on Adil1567/dpef-agent-poc and state transitioned pr_open → awaiting_review. This run also surfaced and fixed a real gap: SKILL.md referenced recording pr_number/pr_url via a vague "write_state" instruction with no matching CLI command — added `state_helper.py set-pr-info` and updated SKILL.md to call it concretely.
- [x] 7.3 Manually verify: two features tracked concurrently do not interfere with each other's state files — confirmed live via `state_helper.py` (two features created, one transitioned, other unaffected) and by `test_multiple_features_independent` in the pytest suite
- [x] 7.4 Manually verify: the agent never merges a PR under any circumstance — confirmed by grep: no merge tool/action referenced anywhere in dpef-build-feature/SKILL.md or state_helper.py; also enforced by branch protection now requiring 1 approving review before merge is even possible
- [x] 7.5 Manually verify: closing a PR without merging transitions state to `rejected` and leaves the branch in place — SKILL.md step 4's rejection branch only calls `transition ... rejected`; confirmed by grep that no delete-branch/rollback/revert action exists anywhere in the skill or state helper
- [x] 7.6 Manually verify: `dpef:init` run a second time reports existing config rather than overwriting it — confirmed: `dpef-agent/config.yaml` exists from the first run, and re-invoking the skill's step 2 branches to the "if it exists: display + ask before redoing" path, not the fresh-scaffold path
- [x] 7.7 Manually verify: a PR whose committed state file deliberately disagrees with GitHub's real merge status shows a failing required check and cannot be merged — confirmed live: pushed a state file to PR #1 falsely claiming `status: merged`, the `verify-state` check ran automatically and reported `conclusion: failure`, and GitHub reports `mergeable_state: blocked` on the PR as a direct result of branch protection
